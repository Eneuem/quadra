                            ...
                            <div x-show.transition.in="step === 1">
                                <form action="submit">
                                    <div class="mb-5">
                                        ...
                                        <input type="login" x-ref="loginField" x-model='login' class="w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium" placeholder="Enter your login..." required>
                                    </div>
                                    <div class="mb-5">
                                        <label for="email" class="font-bold mb-1 text-gray-700 block">Email</label>
                                        <input type="email" ref="emailField" x-model="email" class="w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium" placeholder="Enter your login..." required>
                                    </div>
                                </form>
                            </div>
...
<script>
    function app() {
        return {
            ...
            checkStepOne() {
                const loginField = this.$refs.loginField;
                const emailField = this.$refs.emailField;

                if (!loginField.value.trim() || !emailField.value.trim()) {
                    // Si l'un des champs est vide, ne pas passer à l'étape suivante
                    return;
                }

                // Tous les champs sont remplis, passer à l'étape suivante
                this.step++;
            },
        }
    }
