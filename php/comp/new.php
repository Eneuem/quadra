...
try {
    $featuredMoviesStmt = $pdo->query("SELECT m.id, m.imdb_id, m.title, m.poster_url, fv.position FROM movies m JOIN films_new fv ON m.id = fv.movie_id ORDER BY fv.position ASC");
...
<div class="bg-black rounded-t-lg">
    <h1 class="text-4xl text-center text-gray-300 mb-0 text-bold bg-black py-2 rounded-t-xl">Soon in our cinema :</h1>
