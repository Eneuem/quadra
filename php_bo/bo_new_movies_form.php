...
try {
    $featuredMoviesStmt = $pdo->query("SELECT m.id, m.title, fv.position FROM movies m JOIN films_new fv ON m.id = fv.movie_id ORDER BY fv.position ASC");
...
<form action="add_new_movie.php" method="post" class="p-4 w-full mx-auto mt-4">
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <select id="position" name="position" required class="w-full px-3 py-2 border rounded-lg text-black">
            <?php for ($i = 1; $i <= 6; $i++) : ?>
            ...
            <?php foreach ($featuredMovies as $movie) : ?>
                <tr>
                    ...
                    <td class="border px-4 py-2">
                        <form action="delete_new_movie.php" method="post">
